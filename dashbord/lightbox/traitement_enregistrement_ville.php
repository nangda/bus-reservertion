...
$titre=utf8_decode(mysql_real_escape_string  ($_POST["titre-ville"]));
$reference=utf8_decode(mysql_real_escape_string  ($_POST["reference-ville"]));
$idregion=utf8_decode(mysql_real_escape_string  ($_POST["region-ville"]));
...
if($trouve==false){ 
	if($idville==0){
		 ...
		 $lville=getvilleinformations(mysql_insert_id());
		  ...
		  $class="";
		  $title="Valider "; 
			$url_detail="javascript:Affichage_contenuLightBox64('".base64_encode("lightbox/main.php?idville=$ville[0]&traitement=detail-ville")."";
			...
			$url_valider="javascript:Valider_ville('".base64_encode("traitement_ajax/main.php?traitement=valider-ville")."', '$lville[0]', '".$_SESSION["token"]."', '". $ville["etat"]."')";
		 ...
		 $ville="<tr id=\"tr_ville$lville[0]\">
							...
							<td id=\"nom-ville$lville[0]\">".utf8_encode($lville["titre"])." </td>
		 ...
		 $response["nom"] = utf8_encode($lville["titre"]);
		 $response["reference"] = utf8_encode($lville["reference"]);
		 $response["region"] =utf8_encode($laregion["titre"]);
	}
