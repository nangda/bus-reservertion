...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest' AND isset($_SESSION['token']) AND isset($_POST['token']) AND !empty($_SESSION['token']) AND !empty($_POST['token'])) {
    ...
    if ($_SESSION['token'] == $_POST['token']) { 
		...
		if($laprovince==false){
			$trouve=true;
			$erreur="Aucune province ne correspont";
		}else{
		...
		if($trouve==false){ 
 		   ob_start();
		   include("datatable-province-carte.php");
		   ...
		   $response["next"] = base64_encode("lightbox/lightbox-histogramme-province-carte.php?idprovince=$laprovince[0]");
