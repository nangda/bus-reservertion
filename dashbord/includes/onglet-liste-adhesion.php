								<div class="wrap-contain-onglet-page" id="tab-adhesion" style="display: block;">
									<div class="contain-onglet-page">
										<h2 class="titre-tab">Liste des adh&eacute;sions</h2>
										...
										<div id="wrap-table-adhesion" class="conteneur-wrap-tab">
											...
											<div class="contain-tab2" id="tab-adhesion" style="display: block">
												...
												<div class="wrap-content-page">
													<p class="accroche-tab">Ci-dessous la liste des adh&eacute;sions au mouvement Gabon Nouveau.</p>
                                                <?php 
												   $pagination=7;
												   $page_encours=1;
												   $marge=3;
												   $nbligneparpage=10;
												   $debut=0;
												   $query_membre="SELECT * FROM membre  ORDER BY idmembre DESC";
												   $result_membre=mysql_query($query_membre);
												   $nbreponse=mysql_num_rows($result_membre);
												   $nbtotalpage=ceil($nbreponse/$nbligneparpage);
												   $query_membre.=" LIMIT $debut, $nbligneparpage";
												   $result_membre=mysql_query($query_membre);
												?>
																...
																<div class="form-group">
																	<label for="">Pays
																		<select name="pays" id="-search-pays" type="text" class="search ref-search" onchange="">
																			<option value="" desable selected>Pays de r&eacute;sidence</option>
													...
													<table id="liste-page">
														<thead>
															<tr>
																...
																<th></th>
																<th></th>
																<th align="left">Date d'adh&eacute;sion</th>
														...
														<?php 
	                                                        $i=0;
															while($membre=mysql_fetch_array($result_membre)){
                                                            if($membre["sexe"]=="Homme")$civilite="M."; else $civilite="Mme.";
															$annee_min=substr($membre["date_creation"],2,2);
														    $matricule="A".$membre["idmembre"].$annee_min;
															...
															if($membre["etat"]=="0"){
																...
																$i++;
																$url_supprimer="javascript:Notification_suppression_membre('".base64_encode("traitement_ajax/traitement_supprimer_membre.php")."', '$membre[0]', '".$_SESSION["token"]."')";
													      ?>
															<tr id="tr_membre<?php echo $membre[0]?>">
																<td class="colonne-tab edit-line"><i class="fa fa-pencil"></i></td>
																<td class="colonne-tab detail-line"><a href="javascript:Affichage_contenuLightBox64('<?php echo base64_encode("lightbox/lightbox-detail-membre.php?idmembre=$membre[0]")?>','loader');"><i class="fa fa-eye"></i></a></td>
																<td class="colonne-tab <?php echo $class?>"><i class="fa fa-check-circle"></i></td>
																<td class="colonne-tab delete-line"><a href="<?php echo $url_supprimer?>"><i class="fa fa-trash"></a></i></td>
																<td align="left"><?php echo getdatedetaille($membre["date_creation"], $membre["heure_creation"]) ?></td>
																<td align="left"><?php echo $membre["type_membre"]?></td>
																<td align="left"><?php echo $matricule?></td>
																<td align="left">
																	<div class="detail-liste-page">
																		<div class="info-liste-page">&nbsp;
																		  <a href="#" class="link-detail-ref"><?php echo $civilite;?> <?php echo $membre["nom"]." ".$membre["prenom"]?> </a>
																		</div>
																	</div>
																</td>
																<td align="left"><?php echo $membre["nationalite"]?></td>
																<td align="left"><?php echo $membre["age"]?> ans</td>
																<td align="left"><?php echo $membre["pays"]?></td>
																<td align="left"><?php echo $ville ?></td>
																<td align="left"><?php echo $membre["email"]?></td>
																<td align="left">+<?php echo $membre["indicatif"]?> <?php echo $membre["telephone"]?></td>
															</tr>
															<?php }?>
														</tbody>
													</table>
