	...
	$admin=$ladministrateur[0];
	  ...
	  $query_visite="SELECT * FROM visite WHERE type='Admin' ";

      if($idregion!="" ){
		  $idregion=mysql_real_escape_string  ($_POST["region-visite"]);
		  $query_visite.=" AND idregion='$idregion' ";
	  }
      
      if($titre!=""){
		 
		  $query_visite.=" AND titre LIKE '%$titre%' ";
		  $query_visite.=" ORDER BY titre ASC";
	  }else{
