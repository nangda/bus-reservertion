<div class="wrap-contain-onglet-page" id="tab-companies" <?php if(isset($_SESSION["onglet"]) AND $_SESSION["onglet"]=="tab-companies"){?>style="display: block"<?php }?>>
	<div class="contain-onglet-page">
		<h2 class="titre-tab ">Update companies</h2>
		...
		<a href="javascript:Affichage_contenuLightBox64('<?php echo base64_encode("lightbox/main.php?traitement=editer-company")?>','loader',initDatePaiement);" class="btn-add"><i class="fa fa-plus"></i>Add a company</a>
				<?php }?>
					</div>
		</h2>

				<div class="wrap-content-page">
					<p class="accroche-tab">Here is the list of companies</p>
				<?php 
                  $page=10;//nombre d'enregistrement à afficher par page
                  $debut=0;

				  $query_ville="SELECT * FROM company WHERE idcompany!='0' ";

						
				  $nb_enregistrement=mysql_num_rows(mysql_query("$query_ville"));
				  $query_ville.=" ORDER BY idcompany DESC";
				 ...
				 ?>
					...
					<div class="conteneur-recherche-fiche">
						  <form action="" id="form-recherche-companies" class="formulaire-research form-recherche">
                 ...
                 <div id="liste_ville">
					...
					<table id="liste-page">
						...
						<?php 
							...
							while($company=mysql_fetch_array($result_company)){
								
							$region=getregioninformations($company["idregion"]);
							$deja_operationel=villeDejaoperationnel($company[0]);
							...
							if($company["etat"]=="0"){
								...
								$i++;
							if($droit_dacces["editer_ville"]==1)$url_editer="javascript:Affichage_contenuLightBox64('".base64_encode("lightbox/main.php?idcompany=$company[0]&traitement=editer-company")."','loader');";
							...
							if($droit_dacces["valider_ville"]==1)$url_valider="javascript:Valider_ville('".base64_encode("traitement_ajax/main.php?traitement=valider-company")."', '$company[0]', '".$_SESSION["token"]."', '".$company["etat"]."')";
							...
							if($droit_dacces["supprimer_ville"]==1)$url_supprimer="javascript:Notification_suppression_ville('". base64_encode("traitement_ajax/main.php?traitement=supprimer-company")."', '$company[0]', '".$_SESSION["token"]."',' $deja_operationel')";
						  ...
						  <tr id="tr_ville<?php echo $company[0]?>">
						    <?php if($droit_dacces["editer_ville"]==1){?>
								<td class="colonne-tab edit-line"><a href="<?php echo $url_editer?>"><i class="fa fa-pencil"></i></a>
								</td>
							<?php }?>
								<td class="colonne-tab detail-line"><a href="javascript:Affichage_contenuLightBox64('<?php echo base64_encode("lightbox/main.php?idcompany=$company[0]&traitement=detail-company")?>','loader');"><i class="fa fa-eye"></i></a></td>
								<?php if($droit_dacces["valider_ville"]==1){?>
								<td class="colonne-tab <?php echo $class?>" id="valider-company<?php echo $company[0]?>"><a href="<?php echo $url_valider?>"><i class="fa fa-check-circle"></i></a>
								</td>
								<?php }?>
								...
								<?php }?>
								<td id="titre-company<?php echo $company[0]?>"><?php echo $company["titre"]?></td>
								<td id="reference-company<?php echo $company[0]?>"><? echo $company["reference"]; ?></td>
								<td id="headquater-company<?php echo $company[0]?>"><? echo $company["headquater"]; ?></td>
								<td id="phone-company<?php echo $company[0]?>"><? echo $company["phone"]; ?></td>
								<td id="email-company<?php echo $company[0]?>"><? echo $company["email"]; ?></td>
