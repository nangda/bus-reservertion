<div class="wrap-contain-onglet-page" id="tab-buses" <?php if(isset($_SESSION["onglet"]) AND $_SESSION["onglet"]=="tab-buses"){?>style="display: block"<?php }?>>
	<div class="contain-onglet-page">
		<h2 class="titre-tab ">Updte Buses</h2>
		...
		<a href="javascript:Affichage_contenuLightBox64('<?php echo base64_encode("lightbox/main.php?traitement=editer-ville")?>','loader',initDatePaiement);" class="btn-add"><i class="fa fa-plus"></i>Add a bus</a>
				<?php }?>
					</div>
		</h2>

				<div class="wrap-content-page">
					<p class="accroche-tab">Here is the list of buses</p>
				<?php 
                  $page=10;//nombre d'enregistrement à afficher par page
                  $debut=0;

				  $query_ville="SELECT * FROM bus WHERE idbus!='0' ";

						
				  $nb_enregistrement=mysql_num_rows(mysql_query("$query_ville"));
				  $query_ville.=" ORDER BY idbus DESC";
					...
					<table id="liste-page">
						...
						<tbody id="tbody-ville">
							...
							<tr id="tr_ville<?php echo $ville[0]?>">
								...
								<?php }?>
								<td id="matricule-bus<?php echo $ville[0]?>"><?php echo $bus["matricule"]?></td>
								<td id="reference-bus<?php echo $ville[0]?>"><? echo $bus["reference"]; ?></td>
								<td id="company-bus<?php echo $ville[0]?>"><? echo $bus["company"]; ?></td>
								<td id="bus-seats<?php echo $ville[0]?>"><? echo $bus["seats"]; ?></td>
